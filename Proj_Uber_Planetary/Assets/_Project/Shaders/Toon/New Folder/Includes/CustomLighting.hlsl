...
void AddAdditionalLights_float(float Smoothness, float3 WorldPosition, float3 WorldNormal, float3 WorldView,
    float3 MainDiffuse, float3 MainSpecular, float3 MainColor,
    out float3 Diffuse, out float3 Specular, out float3 Color) {
    Diffuse = MainDiffuse;
    ...
    for (int i = 0; i < pixelLightCount; ++i) {
        Light light = GetAdditionalLight(i, WorldPosition);
        float3 NdotL = saturate(dot(WorldNormal, light.direction));
        float3 atten = light.color * (light.distanceAttenuation * light.shadowAttenuation);
        float3 thisDiffuse = LightingLambert(atten, light.direction, WorldNormal);//atten * NdotL;
